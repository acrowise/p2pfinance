...
if(isset($_POST['current']) && isset($_POST['newpass']) && isset($_POST['confpass'])&& isset($_SESSION['p2pAdm_User']['user_id'])) {

    $current = sha1(trim($_POST['current']));
    $new = trim($_POST['newpass']);
    $conf = trim($_POST['confpass']);
    $up_id = $_SESSION['p2pAdm_User']['user_id'];

    $username = trim($_SESSION['p2pAdm_User']['username']);
    $userEmail = trim($_SESSION['p2pAdm_User']['email']);

    if($new == $conf){
        //if the new password is the same as the confirmation
        $check = new MySQL();
        $check->Query("SELECT * FROM adm_usr_users WHERE user_id = $up_id");
        $recordset = $check->Row();
        ...
        if($current == $dbpass){
            ...
            if(empty($err)){
                ...
                if ($qresult) {
                    ...
                    $emailSubject = "P2P Finance Change Password";
                    //Build email message
